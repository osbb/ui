...
function startAppServer(callback) {
  ...
  appServer.listen(APP_PORT, () => {
    winston.info(`App is now running on http://localhost:${APP_PORT}`);
    if (callback) {
      callback();
    }
  });
...
watcher.on('change', p => {
  winston.info(`\`${p}\` changed. Restarting.`);
  startServers(() =>
    winston.info('Restart your browser to use the updated schema.')
  );
});
startServers();
